...
class MongoDBService {
  ...
  private async autoConnect() {
    ...
    console.log(`baseDbName: ${baseDbName} databaseName: ${databaseName}`);
  ...
  async connect(
    ...
    if (!databaseName) {
      const baseDbName =
        process.env.MONGODB_DATABASE_NAME || "airport_analytics";
      const city = AirportConfigHelper.getConfig().airport.city.toLowerCase();
    ...
    try {
      ...
      console.log(
        "Successfully connected to MongoDB and persisted connection details",
      );
    } catch (error) {
  ...
  async storeSocialEvent(
    ...
    try {
      ...
      if (!existingEvent) {
        ...
        console.log(
          `✅ Stored event to ${sourceName} collection:`,
          event.event_id,
        );
      } else {
        console.log(
          `Event already exists in ${sourceName} collection:`,
          event.event_id,
        );
      }
    } catch (error) {
      console.error(
        `❌ Error storing event to ${sourceName} collection:`,
        error,
      );
      throw error;
    }
  ...
  async bulkStoreSocialEvents(
    ...
    try {
      ...
      console.log(
        `✅ Bulk stored ${result.upsertedCount} new events to ${sourceName} collection`,
      );
    } catch (error) {
      console.error(
        `❌ Error bulk storing events to ${sourceName} collection:`,
        error,
      );
      throw error;
    }
  ...
  async exportCollectionData(sourceName: string): Promise<any[]> {
    try {
      ...
      console.error(
        `Error exporting data from ${sourceName} collection:`,
        error,
      );
      return [];
    }
  ...
  async bulkInsertToCollection(
    ...
    try {
      ...
      console.log(
        `✅ Bulk inserted ${documents.length} documents to ${collectionName} collection`,
      );
    } catch (error) {
      console.error(
        `❌ Error bulk inserting to ${collectionName} collection:`,
        error,
      );
      throw error;
    }
  ...
  async deleteFromCollection(
    ...
    try {
      ...
      console.error(
        `❌ Error deleting from ${collectionName} collection:`,
        error,
      );
      return 0;
    }
  ...
  async getCollectionSize(collectionName: string): Promise<number> {
    try {
      ...
      console.error(
        `❌ Error getting size of ${collectionName} collection:`,
        error,
      );
      return 0;
    }
  ...
  async storeAvaContext(
    sessionId: string,
    context: any,
    userId: string = "Pramit",
  ): Promise<void> {
    if (!this.db) throw new Error("Database not connected");
    const collection = this.db.collection("ava_conversations");
    console.log(
      `📝 [MongoDB] Storing AVA context for session: ${sessionId} | User: ${userId}`,
    );
  ...
  async getAvaContext(sessionId: string): Promise<any> {
    ...
    const collection = this.db.collection("ava_conversations");
    console.log(
      `🔍 [MongoDB] Retrieving AVA context for session: ${sessionId}`,
    );
    ...
    console.log(
      `❌ [MongoDB] No valid context found for session: ${sessionId}`,
    );
    return null;
  }
  ...
  async clearExpiredAvaContexts(): Promise<void> {
    ...
    console.log(
      `🧹 [MongoDB] Cleared ${result.deletedCount} expired AVA contexts`,
    );
  }
}
