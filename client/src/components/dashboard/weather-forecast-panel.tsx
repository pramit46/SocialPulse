...
const weatherIcons = {
  ...
  partly_cloudy: <Cloud className="h-4 w-4 text-muted-foreground" />,
        ...
        <div className="space-y-4">
          {upcomingForecasts.length === 0 ? (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-muted-foreground mt-1">Weather monitoring in progress</p>
            </div>
          ) : (
            upcomingForecasts.map((forecast: WeatherForecast) => (
              <div key={forecast._id} className="bg-muted rounded-lg p-4">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    {weatherIcons[forecast.condition as keyof typeof weatherIcons] || weatherIcons.partly_cloudy}
                    <div>
                      <p className="text-card-foreground font-medium">{forecast.date} at {forecast.time}</p>
                      <p className="text-sm text-muted-foreground">{forecast.temperature}°C • {forecast.condition.replace(/_/g, ' ')}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-xs">
                  <div>
                    ...
                    <p className="text-muted-foreground">{forecast.humidity}%</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Wind Speed</p>
                    <p className="text-muted-foreground">{forecast.windSpeed} km/h</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Visibility</p>
                    <p className="text-muted-foreground">{forecast.visibility} km</p>
                  </div>
                </div>
